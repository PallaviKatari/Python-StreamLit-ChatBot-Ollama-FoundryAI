...
def _load_events(file_path: str = "data/events.txt") -> list:
    ...
    if not os.path.exists(full_path):
        raise FileNotFoundError(f"Missing events file at {full_path}")

    with open(full_path, "r") as f:
        for line in f:
            parts = line.strip().split("|")
            if len(parts) == 4:
                # Expect format: name|type|MM-DD|loc1;loc2
...
def _load_rates(file_path: str) -> dict:
    ...
    if not os.path.exists(full_path):
        raise FileNotFoundError(f"Missing rates file at {full_path}")

    with open(full_path, "r") as f:
        for line in f:
            parts = line.strip().split("|")
            if len(parts) == 2:
                rates[parts[0].lower()] = float(parts[1])
    return rates
...
# Calculate the cost of telescope observation time
